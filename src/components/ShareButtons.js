...
function ShareButtons({ post }) {
  ...
  const handleKakaoShare = () => {
    ...
    if (!window.Kakao.isInitialized()) {
      // JavaScript 키로 초기화
      const kakaoKey =
        process.env.REACT_APP_KAKAO_KEY || "cafd9dfe1955184f28f022dadc403d9b";
    ...
    try {
      window.Kakao.Share.sendDefault({
        ...
        content: {
          ...
          description: post.content
            ...
            : "보험 정보를 확인하세요",
          imageUrl: "/images/cars/placeholder.svg", // 나중에 실제 이미지로 변경
          link: {
      ...
      toast.error("카카오톡 공유에 실패했습니다. 나중에 다시 시도해주세요.");
    }
  };
  ...
  return (
    <div className="share-buttons">
      ...
      {/* 복사 성공 메시지 */}
      {copied && (
        <div className="copy-toast">✅ URL이 클립보드에 복사되었습니다!</div>
      )}
    </div>
  );
}
